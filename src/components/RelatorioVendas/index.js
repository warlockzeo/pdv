...
export default class RelatorioVendas extends Component {
    render() {
        const total = (this.props.dados.length)?this.props.dados.map(evento => parseFloat(evento.pago)).reduce((a,b) => a+b):'0,00';
                ...
                <table className='table table-sm table-hover'>
                    <thead className='relat__head-tabela thead-dark'>
                        <tr style={{display:'flex',flexDirection: 'row', alignItems: 'stretch'}}>
                            <th style={{flex:'0 1 10%'}} className='text-left'>Data</th>
