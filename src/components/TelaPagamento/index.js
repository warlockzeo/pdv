...
export default class TelaPagamento extends Component {
  ...
  pagar = () => {
    const cliente = this.state.clienteAtual.length
      ? this.state.clienteAtual[0].id
      : 0;
  ...
  render() {
    ...
    const clienteGet = (texto) => {
      this.setState({
        clienteAtual: this.props.clientes.filter(
          (cliente) => texto.indexOf(cliente.nome) > -1
        )
      });
    ...
    const cliente = this.state.clienteAtual.length
      ? this.state.clienteAtual[0]
      : '';
    let saldoCliente =
      cliente.saldo > 0 ? (
        <div className="saldo-devedor">Saldo devedor: R$ {cliente.saldo}</div>
      ) : (
        ''
      );
    ...
    const resta =
      this.state.resta > 0
        ? parseFloat(this.state.resta)
            .toFixed(2)
            .replace('.', ',')
        : parseFloat(this.state.resta * -1)
            .toFixed(2)
            .replace('.', ',');

    const restaTroco = (
      <>
        <span className="venda__resta__legenda">
          {this.state.resta >= 0 ? 'Resta:' : 'Troco: '}
        </span>
        <input
          type="text"
          className={
            this.state.resta >= 0
              ? 'venda__resta__input form-control'
              : 'venda__troco__input form-control'
          }
          value={resta}
          disabled
        />
        ...
        <div className="wrap-pagamento">
          ...
          <div className="pagamento col-md-6">
            <div className="pagamento__cliente">
              <Autocomplete
                suggestions={this.props.clientes.map(
                  (cliente) => `${cliente.nome}`
                )}
                callbackParent={(texto) => clienteGet(texto)}
            ...
            <div className="venda__desconto">
              ...
              <input
                type="text"
                className="venda__desconto__input form-control"
                onBlur={onBlur}
                onKeyDown={descontoOnKeyDown}
                onChange={descontoOnChange}
                defaultValue="0,00"
              />
            </div>
            <div className="venda__total">
              ...
              <input
                type="text"
                className="venda__total__input form-control"
                value={parseFloat(this.state.totalAPagar)
                  .toFixed(2)
                  .replace('.', ',')}
                disabled
              />
            ...
            <div className="venda__pago">
              ...
              <input
                type="text"
                className="venda__pago__input form-control"
                onBlur={onBlur}
                onKeyDown={pagoOnKeyDown}
                onChange={pagoOnChange}
                defaultValue="0,00"
              />
            </div>
            <div className="venda__resta">{restaTroco}</div>
            <Button
              className="btn-pagar form-control"
              color="success"
              onClick={this.pagar}>
              Pagar
            </Button>
