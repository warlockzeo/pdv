...
export default class RelatorioProdutos extends Component {
    render() {
        const total = (this.props.dados.length)?this.props.dados.map(evento => parseFloat(evento.subTotal)).reduce((a,b) => a+b):'0,00';
                ...
                <table className='table table-sm table-hover'>
                    <thead className='relat__head-tabela thead-dark'>
                        <tr style={{display:'flex',flexDirection: 'row', alignItems: 'stretch'}}>
                            ...
                            <th style={{flex:'0 1 40%'}} className='text-left'>Produto</th>
                            <th style={{flex:'0 1 15%'}}>Quant</th>
                            <th style={{flex:'0 1 15%'}} className='text-right'>Valor Unit</th>
                            <th style={{flex:'0 1 15%'}} className='text-right'>Subtotal</th>
                        </tr>
                    </thead>
